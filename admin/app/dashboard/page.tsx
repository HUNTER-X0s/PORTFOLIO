...
import { FolderCode, FileText, MessageSquare, Briefcase, Award, TrendingUp, Eye, Star } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs text-gray-300 font-mono uppercase tracking-wider">{label}</p>
          ...
          {messages.slice(0, 5).map((msg: any) => (
            <div key={msg._id} className="px-5 py-3.5 flex items-start gap-3 hover:bg-white/[0.02] transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5 font-mono">{msg.email}</p>
              </div>
...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    { label: 'Resumes', value: stats?.resumes ?? '—', icon: Briefcase, color: '#FF6B2B', sub: 'Role-specific PDFs' },
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div>
        ...
        <p className="text-sm text-gray-300 mt-1">Portfolio CMS &mdash; Anurag Swain</p>
      </div>
