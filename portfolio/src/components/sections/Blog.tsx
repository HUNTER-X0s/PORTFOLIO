    ...
    <div className="py-10 sm:py-14 md:py-20">
      <div className="section-container">
        <div ref={ref as React.RefObject<HTMLDivElement>}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {blogPosts.map((post, i) => (
              <motion.article
                ...
                className="h-full"
              >
                <Dynamic3DCard intensity={14} depth={20} glowColor="rgba(0, 229, 255, 0.25)" className="h-full">
                  <div className="glass-card p-6 flex flex-col group cursor-pointer hover:border-cyan/40 transition-all h-full justify-between">
                    <div>
                      {/* Category */}
                      ...
                      {/* Title */}
                      <h3 className="font-display font-semibold text-text-primary leading-snug mb-3 group-hover:text-cyan transition-colors text-3d-interactive">
                        {post.title}
                      </h3>
          ...
          >
            <button className="flex items-center gap-2 px-6 py-3 rounded-xl mx-auto glass border border-white/[0.09] text-text-secondary hover:text-text-primary hover:border-cyan/30 transition-all text-sm font-medium btn-3d">
              View All Articles
              <ArrowRight size={15} />
            </button>
          </motion.div>
